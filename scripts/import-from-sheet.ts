...
const SHEET_CSV_URL =
  "https://docs.google.com/spreadsheets/d/1FEUf8vFpKmtK-hpnrc3DDoN93dBfOhI2Em8or-T7o6w/export?format=csv";
  ...
  try {
    ...
    for (const sheetProduct of sheetProducts) {
      try {
        ...
        if (!categoryId) {
          log(
            `Skipping "${sheetProduct.item_name}" - category not found`,
            "warn"
          );
          skipped++;
          continue;
        ...
        const product: Product = {
          ...
          description:
            ...
            generateDescription(sheetProduct.item_name, sheetProduct.category),
          price,
          unit_of_measure: "pcs",
          image_url: imageUrl,
          image_alt_text: generateAltText(sheetProduct.item_name),
          image_description: generateDescription(
            sheetProduct.item_name,
        ...
        imported++;
        log(
          `✓ Imported "${sheetProduct.item_name}" (${imported}/${sheetProducts.length})`,
          "success"
        );
      } catch (error) {

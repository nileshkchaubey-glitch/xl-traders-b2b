 ...
 */
export async function parseProductDetailsWithAI(
  text: string,
  categories: string[]
    ...
    try {
      ...
      if (response.ok) {
        ...
        return {
          ...
          quantity_in_unit: parsed.quantity_in_unit
            ? parseInt(parsed.quantity_in_unit)
            : undefined,
      ...
      console.warn(
        "AI Parsing failed, falling back to heuristic parsing:",
        err
      );
    }
  }
...
function parseProductDetailsHeuristically(
  text: string,
  categories: string[]
): ParsedProduct {

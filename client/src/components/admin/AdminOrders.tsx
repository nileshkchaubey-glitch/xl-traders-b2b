...
import { Loader2, ChevronDown, ChevronRight, MessageCircle, Package } from 'lucide-react';
      ...
      >
        ...
        <div className="flex-1 grid grid-cols-2 sm:grid-cols-5 gap-2 text-sm min-w-0">
          ...
          <div onClick={(e) => e.stopPropagation()}>
            ...
            >
              <SelectTrigger className={`h-7 text-xs w-full border-0 px-2 font-semibold rounded-full ${STATUS_COLORS[order.status]}`}>
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
        ...
        <div className="border-t border-slate-100 bg-slate-50 px-4 py-3">
          {loadingItems ? (
            ...
            <div className="space-y-1.5">
              {items.map((item) => (
                <div key={item.id} className="flex items-center gap-3 text-sm bg-white border border-slate-100 rounded-lg px-3 py-2">
                  <span className="font-semibold text-slate-700 w-8 text-center bg-slate-100 rounded px-1 py-0.5 text-xs">{item.quantity}×</span>
                  ...
                  <div className="text-right flex-shrink-0">
                    ...
                    {item.unit_price != null && (
                      <p className="text-xs text-slate-400">@ ₹{Number(item.unit_price).toLocaleString()}</p>
                    )}
                  </div>
                </div>
              ))}
              ...
              <div className="flex justify-end pt-1">
                <span className="text-sm font-bold text-red-600">
                  Total: ₹{items.reduce((s, i) => s + (i.subtotal ?? 0), 0).toLocaleString()}
                </span>
              </div>
            </div>
          )}
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-3">
        ...
        <button onClick={loadOrders} className="text-xs text-slate-500 hover:text-red-600 underline">
          Refresh
        </button>
      </div>
      ...
      <Card className="p-4">
        {loading ? (
          ...
          <div className="text-center py-16 text-slate-400">
            ...
            <p className="text-sm mt-1">Orders placed via the cart will appear here</p>
          </div>
        ) : (

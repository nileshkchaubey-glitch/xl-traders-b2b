  ...
  const handleFileSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (result.rows.length > 0) {
        setStep("preview");
        toast.success(
          `Parsed ${result.rows.length} products from ${selectedFile.name}`
        );
      } else {
        ...
        <Card className="p-6 space-y-6">
          ...
          <div className="rounded-xl border border-green-200 bg-gradient-to-r from-green-50 to-emerald-50 p-5 flex gap-4 items-start">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-sm text-green-700 mt-0.5">
                Download the ready-made XLSX template. It includes sample data,
                an Instructions sheet, and all supported columns (SKU, barcode,
                MOQ, brand…). Fill it in, then upload below.
              </p>
          ...
          <div className="rounded-xl border border-blue-200 bg-gradient-to-r from-blue-50 to-indigo-50 p-5 flex gap-4 items-start">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-sm text-blue-700 mt-1 leading-relaxed">
                ...
                to create size/pack variants.
                <br />
                Leave{" "}
                <strong className="font-semibold text-blue-900">
                  master_name
                </strong>{" "}
                empty for standalone products (existing behavior).
              </p>
            </div>
          </div>
          ...
          <div className="rounded-xl border border-blue-200 bg-gradient-to-r from-blue-50 to-indigo-50 p-5 flex gap-4 items-start">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-sm text-blue-700 mt-1 leading-relaxed">
                <strong className="font-semibold text-blue-900">status</strong>{" "}
                (draft/published, default=draft),{" "}
                <strong className="font-semibold text-blue-900">tags</strong>{" "}
                (business types: restaurant,cloud-kitchen…),{" "}
                <strong className="font-semibold text-blue-900">
                  na_fields
                </strong>{" "}
                (mark fields not applicable: brand,image,specifications)
              </p>
            </div>
          </div>
        ...
        <Card className="p-10 text-center space-y-5">
          ...
          <div>
            ...
            <p className="text-sm text-slate-500 mt-1">
              {progress}% — {Math.round((progress / 100) * parsedRows.length)}{" "}
              of {parsedRows.length} done
            </p>
          </div>
      ...
      {step === "complete" && importResult && (
        <div className="space-y-4">
          ...
          <div className="flex gap-3">
            ...
            <Button onClick={() => onGoToProducts?.()} className="flex-1">

...
export default function AdminImageGallery({
    ...
    try {
      await Promise.all(
        updated
          ...
          .map(img =>
            productImageService.update(img.id, {
    ...
    try {
      ...
      await Promise.all(
        reordered
          ...
          .map(i =>
            productImageService.update(i.id, { display_order: i.display_order })
          )
      );
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      if (autoResize) {
        try {
          ...
          if (saved > 1024)
            toast.success(`Compressed · saved ${formatBytes(saved)}`);
          file = result.file;
        } catch {
  ...
  const saveAltText = async (img: GalleryImage) => {
    ...
    try {
      ...
      setImages(prev =>
        prev.map(i => (i.id === img.id ? { ...i, alt_text: altValue } : i))
      );
      setAltEditing(null);
  ...
  const isPrimary = (img: GalleryImage) =>
    product && img.image_url === product.image_url;
    ...
    >
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
        ...
        <p className="text-xs text-slate-400 -mt-1">
          ...
          Drag cards to reorder. The first image is the primary (shown in
          listings). ⭐ = current primary.
        </p>
          ...
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-16 text-center text-slate-400 border-2 border-dashed border-slate-200 rounded-xl">
              ...
              <p className="text-sm mt-1">
                Upload using the button above or drag &amp; drop images here
              </p>
            </div>
          ) : (

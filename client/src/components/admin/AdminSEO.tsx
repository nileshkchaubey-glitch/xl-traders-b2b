...
async function fetchAllSeoProducts(): Promise<SeoProduct[]> {
  ...
  for (;;) {
    const { data, error } = await supabase
      .from("products")
      .select(
        "id, name, slug, meta_title, meta_description, image_url, price, category_id"
      )
      .order("name")
      .range(from, from + 999);
...
export default function AdminSEO() {
  ...
  // Bulk generation
  const [bulkRunning, setBulkRunning] = useState<"slugs" | "titles" | null>(
    null
  );
  ...
  const saveEdit = async () => {
    ...
    try {
      ...
      if (error) throw error;
      setProducts(prev =>
        prev.map(p => (p.id === editingId ? { ...p, ...update } : p))
      );
      setEditingId(null);
  ...
  const bulkGenerateSlugs = async () => {
    ...
    if (!targets.length) {
      toast.success("All products already have slugs");
      return;
    }
    if (
      !confirm(
        `Auto-generate slugs for ${targets.length} products without one?`
      )
    )
      return;
    ...
    try {
      const taken = new Set(
        products.map(p => p.slug).filter(Boolean) as string[]
      );
      ...
      for (let i = 0; i < updates.length; i += 20) {
        ...
        const results = await Promise.all(
          chunk.map(u =>
            supabase.from("products").update({ slug: u.slug }).eq("id", u.id)
          )
        );
      ...
      setProducts(prev =>
        prev.map(p => (byId.has(p.id) ? { ...p, slug: byId.get(p.id)! } : p))
      );
  ...
  const bulkGenerateTitles = async () => {
    ...
    if (!targets.length) {
      toast.success("All products already have meta titles");
      return;
    }
    if (
      !confirm(
        `Auto-generate meta titles for ${targets.length} products without one?`
      )
    )
      return;
    ...
    try {
      ...
      for (let i = 0; i < updates.length; i += 20) {
        ...
        const results = await Promise.all(
          chunk.map(u =>
            supabase
              .from("products")
              .update({ meta_title: u.meta_title })
              .eq("id", u.id)
          )
        );
        ok += results.filter(r => !r.error).length;
      ...
      setProducts(prev =>
        prev.map(p =>
          byId.has(p.id) ? { ...p, meta_title: byId.get(p.id)! } : p
        )
      );
      ...
      <div className="space-y-4">
        <div>
          ...
          <p className="text-slate-400 text-xs mt-0.5">
            Slugs, meta titles and descriptions
          </p>
        </div>
        <div className="rounded-lg border border-amber-200 bg-amber-50 px-4 py-4 text-sm text-amber-800 space-y-2">
          ...
          <p>
            The <code className="font-mono text-xs">products</code> table
            doesn't have <code className="font-mono text-xs">slug</code>,{" "}
            <code className="font-mono text-xs">meta_title</code> or{" "}
            <code className="font-mono text-xs">meta_description</code> columns
            yet. Run{" "}
            <code className="font-mono text-xs">
              sql/05-product-seo-columns.sql
            </code>{" "}
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="text-slate-400 text-xs mt-0.5">
            {products.filter(p => !p.slug).length} missing slug ·{" "}
            {products.filter(p => !p.meta_title).length} missing meta title
          </p>
        </div>
          ...
          <table className="w-full text-sm min-w-[860px]">
            <thead>
              <tr className="border-b border-slate-200 bg-slate-50">
                <th className="px-3 py-3 text-left font-semibold text-slate-600 text-xs uppercase tracking-wide">
                  Product
                </th>
                <th className="px-3 py-3 text-left font-semibold text-slate-600 text-xs uppercase tracking-wide w-56">
                  Slug
                </th>
                <th className="px-3 py-3 text-left font-semibold text-slate-600 text-xs uppercase tracking-wide w-56">
                  Meta Title
                </th>
                <th className="px-3 py-3 text-left font-semibold text-slate-600 text-xs uppercase tracking-wide w-64">
                  Meta Description
                </th>
                <th className="px-3 py-3 text-left font-semibold text-slate-600 text-xs uppercase tracking-wide w-20">
                  Score
                </th>
                <th className="px-3 py-3 text-left font-semibold text-slate-600 text-xs uppercase tracking-wide w-24">
                  Actions
                </th>
              </tr>
            </thead>
              ...
              {loading ? (
                ...
                pageItems.map(p => {
                  ...
                  return (
                    <tr
                      key={p.id}
                      className={`border-b border-slate-100 transition-colors ${isEditing ? "bg-amber-50/50" : "hover:bg-slate-50/50"}`}
                    >
                      <td className="px-3 py-2 max-w-[220px]">
                        ...
                        <p className="text-xs text-slate-400 truncate">
                          {categoryName(p.category_id) ?? "— uncategorised —"}
                        </p>
                      </td>
                      {isEditing ? (
                        <>

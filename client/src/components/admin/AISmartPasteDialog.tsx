...
import { Sparkles, Loader2, Check, AlertTriangle, HelpCircle } from 'lucide-react';
...
export default function AISmartPasteDialog({ open, onClose, categories, onAutofill }: Props) {
...
Ideal for food packing, microwavable containers."
            ...
            <div className="flex gap-2 p-3 bg-slate-50 border border-slate-200 rounded-xl text-[11px] text-slate-500">
              ...
              <div>
                <p className="font-semibold text-slate-700 mb-0.5">Quick Copy-Paste Tips:</p>
                <p>1. Go to any product page (Amazon, IndiaMART, or other supplier sites).</p>
                <p>2. Select the product details text (or press Ctrl+A to select all) and copy it.</p>
                <p>3. Paste it here. AI will clean up noise and extract the correct values automatically!</p>
              </div>
            </div>

            {/* Action buttons */}
            <div className="flex gap-2 justify-end pt-1">
              <Button variant="outline" size="sm" onClick={onClose} disabled={parsing}>
                Cancel
              </Button>
              <Button
                onClick={handleParse}
          ...
          <div className="space-y-4">
            <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-3 flex items-center gap-2 text-emerald-800 text-xs">
              ...
              <span>AI has extracted the following details! Review and edit if needed.</span>
            </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                ...
                <div className="space-y-1.5">
                  ...
                  <Input
                    ...
                    onChange={(e) => setEditedResult({ ...editedResult, price: e.target.value ? parseFloat(e.target.value) : undefined })}
                    className="h-9 text-sm border-slate-200"
                  />
                </div>
                ...
                <div className="space-y-1.5">
                  ...
                  <Input
                    ...
                    onChange={(e) => setEditedResult({ ...editedResult, mrp: e.target.value ? parseFloat(e.target.value) : undefined })}
                    className="h-9 text-sm border-slate-200"
                  />
                </div>

                <div className="space-y-1.5">
                  <Label className="text-xs font-semibold text-slate-500">Unit of Measure</Label>
                  <Input
                    value={editedResult.unit_of_measure || ''}
                    onChange={(e) => setEditedResult({ ...editedResult, unit_of_measure: e.target.value })}
                    className="h-9 text-sm border-slate-200"
                ...
                <div className="space-y-1.5">
                  <Label className="text-xs font-semibold text-slate-500">Quantity in Unit</Label>
                  <Input
                    type="number"
                    value={editedResult.quantity_in_unit != null ? editedResult.quantity_in_unit : ''}
                    onChange={(e) => setEditedResult({ ...editedResult, quantity_in_unit: e.target.value ? parseInt(e.target.value) : undefined })}
                    className="h-9 text-sm border-slate-200"
                  />
                </div>
                ...
                <div className="space-y-1.5">
                  ...
                  <Input
                    ...
                    onChange={(e) => setEditedResult({ ...editedResult, brand: e.target.value })}
                    className="h-9 text-sm border-slate-200"
                  />
                </div>

                <div className="space-y-1.5">
                  <Label className="text-xs font-semibold text-slate-500">AI Category Suggestion</Label>
                  <Input
                    value={editedResult.category_name || ''}
                    onChange={(e) => setEditedResult({ ...editedResult, category_name: e.target.value })}
                    className="h-9 text-sm border-slate-200"

...
const MISSING_CHIP_META: Record<
  MissingFilter,
...
async function fetchStats(): Promise<Stats> {
  const [
    ...
    supabase
      .from("products")
      .select("id,name,created_at,is_active,image_url,categories(name)")
...
function HealthRow({
  ...
  return (
    <div className="flex items-center gap-3 py-2">
      <span className="w-20 text-sm text-slate-600 flex-shrink-0 font-medium">
        {label}
      </span>
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-slate-500 text-sm mt-0.5">
          XL Traders B2B — catalogue summary
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4">
        ...
        <KpiCard
          ...
          sub={
            stats.newEnquiries > 0 ? `${stats.newEnquiries} new` : "All handled"
          }
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="bg-white rounded-xl border border-slate-200/80 p-6 flex flex-col items-center justify-center text-center">
          ...
          <p className="text-xs text-slate-400 mt-1">
            {stats.totalCategories} categories · {stats.totalProducts} products
          </p>
        </div>
        ...
        <div className="bg-white rounded-xl border border-slate-200/80 p-5">
          ...
          <div className="divide-y divide-slate-50">
            <HealthRow
              label="Images"
              count={stats.totalProducts - stats.missing.image}
              total={stats.totalProducts}
            />
            <HealthRow
              label="Prices"
              count={stats.totalProducts - stats.missing.price}
              total={stats.totalProducts}
            />
            <HealthRow
              label="Category"
              count={stats.totalProducts - stats.missing.category}
              total={stats.totalProducts}
            />
            <HealthRow
              label="Brand"
              count={stats.totalProducts - stats.missing.brand}
              total={stats.totalProducts}
            />
            <HealthRow
              label="Description"
              count={stats.totalProducts - stats.missing.description}
              total={stats.totalProducts}
            />
            <HealthRow
              label="SEO"
              count={stats.totalProducts - stats.missing.seo}
              total={stats.totalProducts}
            />
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl border border-slate-200/80 p-5">
          <div className="flex items-center gap-2 mb-4">
            <AlertTriangle
              className={`w-4 h-4 ${totalIssues > 0 ? "text-amber-500" : "text-emerald-500"}`}
            />
            ...
            {stats.recentProducts.map(p => (
              ...
              >
                ...
                <div className="flex items-center gap-3 flex-shrink-0">
                  <span
                    className={`text-xs px-2 py-0.5 rounded-full font-semibold ${p.is_active ? "bg-emerald-50 text-emerald-700" : "bg-slate-100 text-slate-500"}`}
                  >

...
- **healthService.ts:** Thin service reading `v_product_health` DB view — `getMissingCounts()` returns per-field missing counts; `productHealth(id)` returns full health row; `getProductsMissing(field, page)` stub wired for future filters.

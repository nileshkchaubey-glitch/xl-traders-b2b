...
**Last updated: June 16, 2026** · Update this after every merged PR.
...
- **Import UI truthfulness (June 16, 2026):** Both import screens (Google Sheets + CSV) were updated to match the nullable migration (PR #46). **Only `name` + `unit` are required** — `price` and `category` no longer show a red `*` asterisk (blank price imports as NULL = "Price on enquiry"; blank category auto-assigns Uncategorized). Column chips are now colour-coded: green ✱ = required, white = optional, **blue outline = new/advanced** (`status`, `tags`, `na_fields`, plus `master_name`/`variant_label` on the Sheets screen). The CSV screen gained a second blue info box documenting `status`/`tags`/`na_fields`. The Google Sheets `mapRow` no longer drops blank-price/blank-category rows, and both preview tables render null prices as "On enquiry" instead of crashing on `.toLocaleString()`. **Download Template** (`templateService.ts`) gained `status`, `tags`, `na_fields` columns with sample values + Instructions rows, and a best-effort data-validation dropdown (draft/published) on the `status` column — note the community `xlsx` build may not emit the dropdown, so the Instructions sheet + sample row (status=draft) are the real guardrail.

...
- `node scripts/xl-products.mjs gen-image --prompt "..." --as name.png` (optional; needs IMAGE*GEN*\* in .env)
...
4. **Image.** Priority: (a) URL the owner gives → use as `--image`; (b) a local photo file
   ...
   IMAGE*GEN*\* is configured → `gen-image --prompt "<clean product photo, white background,
